    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card className="flex items-start gap-2">
          ...
          <div>
            <p className="text-base font-extrabold text-ink">2,450</p>
        ...
        <Card className="flex items-start gap-2">
          ...
          <div>
            <p className="text-base font-extrabold text-ink">₹1,250</p>
      ...
      <Card>
        <div className="mb-3 flex items-center justify-between">
          <h3 className="text-sm font-bold text-ink">My Bookings</h3>
        ...
        <div className="grid grid-cols-4 gap-2 text-center text-xs">
          {[
            { label: "Trips", count: 3, icon: <Plane size={18} />, color: "text-primary bg-primary-light" },
            { label: "Flights", count: 2, icon: <Plane size={18} />, color: "text-info bg-info-light" },
            { label: "Hotels", count: 1, icon: <Hotel size={18} />, color: "text-success bg-success-light" },
            { label: "Cabs", count: 0, icon: <Car size={18} />, color: "text-accent bg-accent-light" },
          ].map((b) => (
            <div key={b.label} className="flex flex-col items-center gap-1">
              <div className={`flex size-10 items-center justify-center rounded-full ${b.color}`}>{b.icon}</div>
              <p className="font-bold text-ink">{b.count}</p>
              <p className="text-muted">{b.label}</p>
              <Badge tone="primary" className="text-[9px]">{b.count} Upcoming</Badge>
            </div>
          ))}
        </div>
      </Card>
